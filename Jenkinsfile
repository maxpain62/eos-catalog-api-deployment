  ...
  """
      ...
      node (label) {
          ...
          stage ('Checkout SCM'){
            git credentialsId: 'git', url: 'https://dptrealtime@bitbucket.org/dptrealtime/eos-catalog-api-deployment.git', branch:  "${env}"
          }
          ...
          stage ('Helm Chart') {
            container('build') {
                withCredentials([usernamePassword(credentialsId: 'jfrog', usernameVariable: 'username', passwordVariable: 'password')]) {
                      ...
                      sh "/usr/local/bin/helm upgrade  --install --force catalog-api  --namespace ${env} -f values.yaml eos-helm-local/catalog-api"
